...
/* set this to true to enabel debug */
var debug = false;
...
function Device(driver , lwrfType, lWRFId, lShortName) {
  ...
  {
     this.D = 224;
     console.log('LWRF Registered a light: ' + lWRFId + ' Called ' + lShortName);
     self.emit('on', false);
  }
    else if (lwrfType == "Switch")
  {
     this.D = 207;
     console.log('LWRF Registered a switch: ' + lWRFId+ ' Called ' + lShortName);
     self.emit('on', false);
  }
  else
  {
...
Device.prototype.write = function(data)
{
  ...
  if(self.lwrfType == "Light")
  {
    ...
    if(parsed.raw) {
      sendRawUDPCommand(",!"  +  parsed.raw );
    }
    else if(parsed.bri <= 32)
    {
      sendRawUDPCommand(",!" + self.lWRFId + "FdP" + parsed.bri + "|" + self.name );
    }
    else if(parsed.on == true)
    {
      sendRawUDPCommand(",!" + self.lWRFId + "F1|" + self.name );
    }
    else
    {
      sendRawUDPCommand(",!" + self.lWRFId + "F0|" + self.name );
    }
  }
  else if (self.lwrfType  == "Switch")
  {
    if(parsed.on == true)
    {
      sendRawUDPCommand(",!" + self.lWRFId + "F1|" + self.name );
...
function  sendRawUDPCommand(cmd)
{
  ...
  var message = new Buffer(msgCount.toString() + cmd);
  msgCount++;
  if(msgCount > 999)
  {
    msgCount = 000;
